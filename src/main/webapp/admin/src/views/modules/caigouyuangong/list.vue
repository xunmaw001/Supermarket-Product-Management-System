                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('caigouyuangong','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/chaoshishangpinguanli/upload/caigouyuangongMuBan.xls"
                        >批量导入采购员工数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('caigouyuangong','导入导出')"
                                ...
                                :on-success="caigouyuangongUploadSuccess"
                                :on-error="caigouyuangongUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('caigouyuangong','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入采购员工数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('caigouyuangong','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "caigouyuangong.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('caigouyuangong','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('caigouyuangong','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('caigouyuangong','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>

                            <el-button v-if="isAuth('caigouyuangong','修改')" type="success" icon="el-icon-tickets" size="mini" @click="resetPassword(scope.row.id)">重置密码</el-button>
                        </template>
                    </el-table-column>
                </el-table>
                <el-pagination
...
<script>
    ...
    //当前表下拉框搜索条件
            form:{
                ...
                username : null,
                password : null,
                caigouyuangongUuidNumber : null,
                caigouyuangongName : null,
                caigouyuangongPhone : null,
                caigouyuangongIdNumber : null,
                caigouyuangongPhoto : null,
                sexTypes : null,
                caigouyuangongEmail : null,
                createTime : null,
            },
            dataList: [],
            pageIndex: 1,
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :"caigouyuangong",//当前表表名,
                        sumColum : 'caigouyuangong_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'caigouyuangong_name',//分组字符串字段
                        // types : 'caigouyuangong_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "caigouyuangong_types",
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                params['caigouyuangongDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "caigouyuangong/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            resetPassword(id) {
                //重置密码
                this.$http({
                    url: "caigouyuangong/resetPassword?id=" + id,
                    method: "get",
                    // id:id
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            caigouyuangongUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "caigouyuangong/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            // 导入功能上传文件失败后调用导入方法
            caigouyuangongUploadError(data){
