        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '退货单号' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.caigoutuihuoUuidNumber" placeholder="退货单号" clearable></el-input>
                     </el-form-item>
         
                    <el-form-item :label="contents.inputTitle == 1 ? '开始退货数量' : ''">
                        <el-input v-model="searchForm.caigoutuihuoNumberStartSearch"
                               placeholder="请输入开始退货数量" clearable></el-input>
                        </el-form-item>
                        <el-form-item :label="contents.inputTitle == 1 ? '' : ''">
                        =》
                        </el-form-item>
                        <el-form-item :label="contents.inputTitle == 1 ? '结束退货数量' : ''">
                        <el-input v-model="searchForm.caigoutuihuoNumberEndSearch"
                               placeholder="请输入结束退货数量" clearable></el-input>
                    </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('caigoutuihuo','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/chaoshishangpinguanli/upload/caigoutuihuoMuBan.xls"
                        >批量导入采购退货数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('caigoutuihuo','导入导出')"
                                ...
                                :on-success="caigoutuihuoUploadSuccess"
                                :on-error="caigoutuihuoUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('caigoutuihuo','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入采购退货数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('caigoutuihuo','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "caigoutuihuo.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="caigoutuihuoUuidNumber"
                                   header-align="center"
                                   label="退货单号">
                        <template slot-scope="scope">
                            {{scope.row.caigoutuihuoUuidNumber}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="caigoutuihuoNumber"
                                      header-align="center"
                                      label="退货数量">
                        <template slot-scope="scope">
                            {{scope.row.caigoutuihuoNumber}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('caigoutuihuo','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('caigoutuihuo','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('caigoutuihuo','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            form:{
                ...
                caigoutuihuoUuidNumber : null,
                caigoutuihuoNumber : null,
                caigoutuihuoContent : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                ...
                //本表字段
                     '退货单号': "caigoutuihuoUuidNumber",
                     '退货数量': "caigoutuihuoNumber",
                     '退货时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :"caigoutuihuo",//当前表表名,
                        sumColum : 'caigoutuihuo_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'caigoutuihuo_name',//分组字符串字段
                        // types : 'caigoutuihuo_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "caigoutuihuo_types",
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                if (this.searchForm.caigoutuihuoUuidNumber!= '' && this.searchForm.caigoutuihuoUuidNumber!= undefined) {
                    params['caigoutuihuoUuidNumber'] = '%' + this.searchForm.caigoutuihuoUuidNumber + '%'
                }
         

                if (this.searchForm.caigoutuihuoNumberStartSearch != '' && this.searchForm.caigoutuihuoNumberStartSearch != undefined) {
                    params['caigoutuihuoNumberStart'] = this.searchForm.caigoutuihuoNumberStartSearch
                }
                if (this.searchForm.caigoutuihuoNumberEndSearch != '' && this.searchForm.caigoutuihuoNumberEndSearch != undefined) {
                    params['caigoutuihuoNumberEnd'] = this.searchForm.caigoutuihuoNumberEndSearch
                }
                        
                params['caigoutuihuoDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "caigoutuihuo/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            caigoutuihuoUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "caigoutuihuo/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
